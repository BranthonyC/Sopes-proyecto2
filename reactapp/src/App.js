...
export default class App extends Component {
  ...
  async getDataAxios(){
      const response = await axios.get("http://10.4.4.244:8081/");
      this.setState({last_item:response.data});
      console.log(response.data);
  }

  async getPrimerReporte(){
    let response = await axios.get("http://10.4.4.244:8081/primero");
  ...
  async getSegundoReporte(){
    let response = await axios.get("http://10.4.4.244:8081/segundo");
    let info = response.data.map(resitem=>{
      let randomColor = Math.floor(Math.random()*16777215).toString(16);
      let item = {}
      item.title=resitem._id;
  ...
  async getCuartoReporte(){
    let response = await axios.get("http://10.4.4.244:8081/cuarto");
